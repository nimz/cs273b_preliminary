...
loader = load_dataset.DatasetLoader(chromosome=21, windowSize=config.window,
                                    ...
                                    seed=1, test_frac=0.025, pos_frac=0.5, load_coverage=False, load_entropy=True)

conv_net = SimpleConvCoverage(config, loader, include_entropy = True)
...
auroc = conv_net.calc_roc(sess, 'conv_auroc_entropy.png')
print("ROC AUC: %g" % auroc)
auprc = conv_net.calc_auprc(sess, 'conv_auprc_entropy.png')
print("PR AUC: %g" % auprc)
print("f1 score: %g" % conv_net.calc_f1(sess))
conv_net.print_confusion_matrix(sess)

conv_net.plot_val_accuracies('conv_val_entropy.png')
